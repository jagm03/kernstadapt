...
```{r, eval = FALSE}
each.time <- lapply(adapt.dens, as.data.frame)
merged.times <- do.call("rbind", each.time)
merged.times$Time <- rep(1:64, rep(length(each.time[[1]]$x), 64))
Wpoints <- as.data.frame(window(amazon))

library(gganimate)
ANI <- ggplot() +  #7.5*1.64in
  geom_raster(data = merged.times, aes(x, y, fill = log(value))) +     
